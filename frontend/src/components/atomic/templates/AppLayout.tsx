        ...
        <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8 dark:border-gray-700 dark:bg-gray-900/80">
          ...
          <div className="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
            ...
            <div className="flex items-center gap-x-4 text-gray-900 lg:gap-x-6 dark:text-gray-200">
              {(queryClient.isMutating() > 0 || queryClient.isFetching() > 0) && <PiSpinner size={14} className={queryClient.isMutating() > 0 ? 'animate-ping' : 'animate-spin'} />}
              <Dropdown
                menu={{
              ...
              >
                <button type="button" className={twMerge('-m-2.5 p-2.5 text-gray-400 hover:text-gray-500', unreadNotification && 'animate-ping text-primary-500 hover:animate-none')}>
                  <span className="sr-only">View notifications</span>
                  {unreadNotification ? <PiNotificationBold className="h-6 w-6" aria-hidden="true" /> : <PiNotification className="h-6 w-6" aria-hidden="true" />}
                </button>
              </Dropdown>
