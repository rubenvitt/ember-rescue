...
export function DynamicGrid<T extends Identifiable>({
  ...
  const renderItems = useMemo(() => {
    return items.map((item, index) => {
      ...
      const span = isLastItem ? (remainingItems === 1 ? 'full' : remainingItems === 2 ? 'two' : 'default') : 'default';
  ...
  return (
    <div className={`grid auto-rows-auto grid-cols-1 justify-items-center gap-4 md:grid-cols-${columns}`}>
      {renderItems}
    </div>
  );
}
