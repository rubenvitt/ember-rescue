...
export function MinimalDotsDropdown({ title, dropdownItems, iconSize = 20 }: MinimalDotsDropdownProps) {
  const buttonContent = useMemo(() => (
    <>
      ...
      <PiCaretDown size={iconSize} aria-hidden="true" />
    </>
  ), [title, iconSize]);
  ...
  return (
    <GenericDropdown
      ...
      buttonClassName="p-1 rounded-full dark:text-white hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
